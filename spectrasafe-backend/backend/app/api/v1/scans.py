...
_require_shopkeeper = require_role(UserRole.SHOPKEEPER)
_require_shopkeeper_or_fssai = require_role(
    UserRole.SHOPKEEPER,
    UserRole.FSSAI,
)
...
def create_scan(
    ...
    # Coordinates supplied by LocationAutocomplete
    ...
    if suffix not in _ALLOWED_SUFFIXES:
        raise HTTPException(
            400,
            f"Unsupported file type '{suffix}'. Use jpg/jpeg/png.",
        )
    ...
    violation_types = classify_violation_types(
        compliance
    )

    verification = verify_against_registry(
        extracted,
        db,
    )
    ...
    #
    # Prefer coordinates selected by LocationAutocomplete.
    # Only geocode the text if coordinates were not supplied.
    # --------------------------------------------------------
    ...
    scan = Scan(
        ...
        fssai_license=extracted.get(
            "fssai_license"
        ),

        matched_product_id=verification.get(
            "matched_product_id"
        ),
...
def rescan_license(
    ...
    try:
        license_value = ocr_pipeline.rescan_fssai_license(
            tmp_path
        )
    ...
    if not license_value:
        raise HTTPException(
            422,
            "No FSSAI license number detected in this photo. "
            "Try a clearer, closer shot of just the license text.",
        )

    # Recompute compliance/verification with corrected license.
    extracted = dict(scan.ocr_result)
    ...
    scan.compliance_score = compliance["score"]
    scan.violation_types = violation_types
    scan.matched_product_id = verification.get(
        "matched_product_id"
    )
    scan.product_verification = verification
...
def create_batch_scan(
    ...
    # --------------------------------------------------------
    # Determine coordinates ONCE for the entire batch.
    #
    # If LocationAutocomplete provided coordinates,
    # use them directly.
    #
    # Otherwise geocode the location text once.
    # --------------------------------------------------------
    ...
    for file in files:
        ...
        try:
            ...
            extracted = ocr_pipeline.run_pipeline(
                tmp_path
            )
    ...
    # --------------------------------------------------------
    # Return successful scans + errors
    # --------------------------------------------------------
    ...
    return JSONResponse(
        ...
        headers={
            "X-Batch-Errors": json.dumps(errors)
        },
    )
...
def list_scans(
    ...
    if current_user.role == UserRole.SHOPKEEPER:
        query = query.filter(
            Scan.scanned_by_user_id == current_user.id
        )
...
def get_scan(
    ...
    if (
        ...
        and scan.scanned_by_user_id != current_user.id
    ):
        raise HTTPException(
            404,
            "Scan not found",
