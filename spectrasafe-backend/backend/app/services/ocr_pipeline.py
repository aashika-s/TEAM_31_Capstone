...
# def detect_per100_value_index(tokens: list[dict]) -> int | None:
#     """When per-serving and per-100g headers got OCR'd as a single glued
#     token (no gap detected between them, so detect_column_positions ends
#     up with both at ~the same x-position and choose_target_column can't
#     trust either), x-position can't tell us which physical column is
#     which -- but the token's own text can, since word order in the glued
#     string mirrors left-to-right column order on the label (e.g. "Avg.
#     Quantity per serving AvgQuantity per 100g" -> serving column first,
#     so the per-100g value is the SECOND number on each row).

#     Returns the index (0 or 1) of the per-100g value among a row's
#     numbers, or None if no glued header token was found -- callers should
#     treat None the same as index 0 (existing "take the first number"
#     behavior), since that's correct whenever there's really only one
#     column's worth of numbers per row."""
#     for t in tokens:
#         text = t["text"]
#         m_serving = _COLUMN_HEADER_PATTERNS["per_serving"].search(text)
#         m_100 = _COLUMN_HEADER_PATTERNS["per_100"].search(text)
#         if m_serving and m_100:
#             return 1 if m_serving.start() < m_100.start() else 0
#     return None
...
# def _pick_nutrient_value(row, value_index: int = 0):
#     """Returns (picked_match, first_match) -- picked_match is the value at
#     value_index (what column-order detection told us to use), first_match
#     is always index 0 regardless of value_index. Callers need both: the
#     label-key boundary must stop at the FIRST number on the row (the
#     label always precedes both column values), not at whichever one got
#     picked as the value -- otherwise a skipped earlier column value ends
#     up glued onto the label text instead of being dropped."""
#     matches = list(_NUTRIENT_VAL.finditer(row))
#     if not matches: return None
#     non_pct = [m for m in matches if not m.group(1).strip().endswith('%')]
#     if not non_pct: return None
#     idx = min(value_index, len(non_pct) - 1)
#     return non_pct[idx], non_pct[0]

# def _parse_nutrient_row(row, value_index: int = 0):
#     row = _fix_ocr(row)
...
#     if m_label_unit:
#         nums_list = m_label_unit.group("nums").split()
#         if not nums_list: return None
#         idx = min(value_index, len(nums_list) - 1)
#         return (_norm_label(m_label_unit.group("label").strip()), nums_list[idx].strip()) if len(m_label_unit.group("label").strip()) >= 2 else None

#     m = _pick_nutrient_value(row, value_index)
#     if not m:
...
#             # value.
#             num_matches = list(re.finditer(r'(?<!\()\b\d+(?:\.\d+)?\b(?!\))', row))
#             if num_matches:
#                 picked = num_matches[min(value_index, len(num_matches) - 1)]
#                 label_text = row[:picked.start()].strip() if value_index == 0 else row[:num_matches[0].start()].strip()
#                 return (_norm_label(label_text), picked.group())
#         return None

#     picked, first = m
#     key = re.sub(r'\s{2,}', ' ', re.sub(r'[:\-–|]+$', '', row[:first.start()]).strip())
#     return (_norm_label(key), picked.group(1).strip()) if len(key) >= 2 else None
...
# def parse_nutrient_table(rows, value_index: int = 0):
#     nutrients, unmatched = {}, []
#     for row in rows:
#         m_size = re.search(r'serving\s*size\s*[:\-]?\s*(\d+(?:\.\d+)?\s*(?:g|ml|tbsp|tsp|cup)?)', row, re.IGNORECASE)
...
#         r = _parse_nutrient_row(row, value_index)
#         if r:
#             key, val = r
#             # Classify using key+value together, not just the key -- in
#             # glued multi-column rows the label often carries no unit at
#             # all (just "Energy", not "Energy (kJ)"), with the unit only
#             # attached to each value instead. Key-only classification then
#             # defaults to the kcal bucket's narrow (0-900) range even for
#             # a genuine kJ reading, and _correct_decimal_drop "corrects" a
#             # perfectly valid 1969 down to 196.9.
#             bucket = _classify_nutrient(f"{key} {val}")
...
# def structure_tokens(label, rows, value_index: int = 0):
...
#                 val = m.group(1) or m.group(2) or m.group(3)
#                 if val: ins.append(re.sub(r'\([ivxIVX]+\)$', '', val).strip())
#         out["nutrients"]   = parse_nutrient_table(rows, value_index)
#         out["ins_numbers"] = sorted(set(ins))
...
#             target_col = choose_target_column(col_pos)
#             if target_col:
...
#             else:
#                 # Column filtering was skipped (no columns detected, or
#                 # per_100/per_serving collapsed onto the same x-position
#                 # from one glued header token) -- rows will carry both
#                 # columns' numbers glued together. If that glued header
#                 # token's own text tells us which column comes first, use
#                 # it to pick the right one instead of defaulting to
#                 # "first number on the row", which is only correct when
#                 # per-100g happens to be listed first.
#                 detected_idx = detect_per100_value_index(tokens)
#                 if detected_idx is not None:
#                     value_index = detected_idx

#         structured = structure_tokens(region["label"], reconstruct_rows(tokens, label=region["label"]), value_index)
...
# can live inside a deployed service rather than a notebook working directory
# ══════════════════════════════════════════════════════════════════════════════
YOLO_WEIGHTS          = os.getenv("YOLO_WEIGHTS_PATH", "best (1).pt")
YOLO_CONF             = 0.50
FSSAI_CONF            = 0.30    # Lowered specifically for FSSAI boxes
OCR_CONF_GATE         = 0.55
OCR_CONF_SOFT_FLOOR   = 0.40
...
def check_trans_fat_compliance(nutrients: dict) -> dict:
    ...
    IMPORTANT LIMITATION: FSSAI Regulation 2.3.14(21) sets its 2% limit
    against "total oils/fats present in the product" -- a formulation-
    level quantity that is NOT printed on any nutrition label and cannot
    be extracted by OCR. What we actually have is the label's declared
    "Total Fat" (a nutritional-analysis value), which is a different
    basis, not a substitute for it. This check computes trans fat as a
    percentage of declared Total Fat as the closest available proxy, and
    says so explicitly in its own output -- it must never be presented as
    the regulation's exact figure."""
    trans_val = total_fat_val = None
    for key, val in nutrients.items():
        bucket = _classify_nutrient(key)
        if bucket == "trans_fat" and trans_val is None: trans_val = val
    ...
    return {
        "status": "compliant" if pct <= 2.0 else "exceeds_limit",
        "trans_fat_pct_of_declared_total_fat": round(pct, 2),
        "limit_pct": 2.0,
        "regulation": "FSS (Prohibition and Restriction on Sales) Regulations, 2011 — Reg. 2.3.14(21)",
        "regulation_basis": (
            "Regulation 2.3.14(21) sets its 2% limit against total oils/fats "
            "used as an ingredient in the product -- a formulation quantity "
            "not printed on any label. This figure is trans fat as a percentage "
            "of the label's DECLARED Total Fat instead, the closest thing "
            "available from a scanned nutrition panel. Treat it as an "
            "approximation, not the regulation's exact basis, and note that "
            "FSSAI's compendia are reference documents -- the original Gazette "
            "notification prevails on any discrepancy."
        ),
    }
...
def rescan_fssai_license(image_path):
    """
    Re-runs region detection + OCR on a new photo aimed at capturing the
    FSSAI license -- for when the license printed somewhere away from
    the nutrition/ingredients panel wasn't captured or wasn't legible in
    the original scan. Only the 'fssai' region's output is used; any
    other regions this new photo happens to trigger are ignored.
    Returns the extracted license string, or None if nothing usable
    was found.
    """
    global _yolo_model_cache, _ocr_engine_cache
    if _yolo_model_cache is None or _ocr_engine_cache is None:
    ...
    regions = detect_regions(image_path, _yolo_model_cache, ocr_engine=_ocr_engine_cache)
    fssai_regions = [r for r in regions if "fssai" in r["label"].lower()]
    if not fssai_regions:
        return None

    best_value = None
    for region in fssai_regions:
        tokens = line_segmented_ocr(region["crop_bgr"], _ocr_engine_cache, gate=OCR_CONF_GATE, label=region["label"])
        structured = structure_tokens(region["label"], reconstruct_rows(tokens, label=region["label"]), 0)
        value = structured.get("fssai_license")
        if not value:
            continue
        # Prefer a clean 14-digit read over a partial/garbled one, but
        # keep any read as a fallback rather than returning nothing --
        # a shopkeeper can still see and manually verify a rough OCR
        # result, which is strictly better than silence.
        if re.search(r'\b\d{14}\b', value):
            return value
